/**
 * API Route: /api/sell
 * Vercel Serverless Function - Execute sell transactions
 */

import type { VercelRequest, VercelResponse } from '@vercel/node';
import { 
  executeSellTransaction, 
  executeSellWithConfirmation,
  BuySellConfig 
} from '../model/buy-sell';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    console.log(`📥 /api/sell request: ${amountFloat} ${inputMint} → ${outputMint}`);
    ...
    if (executeNow && process.env.SOLANA_PRIVATE_KEY) {
      ...
      const result = await executeSellWithConfirmation(
        buySellRequest,
        TRANSACTION_CONFIG,
