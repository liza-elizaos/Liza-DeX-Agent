 ...
 * Usage: node test-free-rpc-simple.js
 */
...
async function runTests() {
  ...
  await logTest('Basic Connection', async () => {
    const connection = new Connection(FREE_RPC, 'confirmed');
    ...
    for (let i = 0; i < Math.min(signatures.length, 50); i++) {
      try {
        ...
        if (tx) {
          ...
          // Check for token launches in instructions
          try {
            const instructions = tx.transaction.message.instructions || [];
            const hasInitializeMint = instructions.some((instr) => {
