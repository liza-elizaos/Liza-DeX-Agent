...
async function runTests() {
    ...
    await logTest('Basic Connection', async () => {
        const connection = new web3_js_1.Connection(FREE_RPC, {
    ...
    await logTest('Get Wallet Balance', async () => {
        ...
        const pubkey = new web3_js_1.PublicKey(TEST_WALLET);
        ...
        for (let i = 0; i < Math.min(signatures.length, 50); i++) {
            try {
                ...
                if (tx) {
                    ...
                    const hasInitializeMint = tx.transaction.message.instructions.some((instr) => {
